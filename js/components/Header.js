// ==================== HEADER COMPONENT ====================

function NavigationLink({ item, currentPage, setCurrentPage }) {
    const handleClick = (e) => {
        if (item.action) {
            e.preventDefault();
            setCurrentPage(item.action);
            window.scrollTo(0, 0);
        } else if (item.external) {
            return;
        } else if (item.href && item.href.startsWith('#')) {
            e.preventDefault();
            if (currentPage !== 'home') {
                setCurrentPage('home');
                setTimeout(() => {
                    const element = document.querySelector(item.href);
                    if (element) element.scrollIntoView({ behavior: 'smooth', block: 'start' });
                }, 100);
            } else {
                const element = document.querySelector(item.href);
                if (element) element.scrollIntoView({ behavior: 'smooth', block: 'start' });
            }
        }
    };

    if (item.action) {
        return React.createElement(
            'button',
            { onClick: handleClick, className: 'text-gray-400 hover:text-emerald-500 transition-colors' },
            item.label
        );
    }

    if (item.external) {
        return React.createElement(
            'a',
            {
                href: item.href,
                target: '_blank',
                rel: 'noopener noreferrer',
                className: 'px-3 py-1.5 border border-emerald-500/50 text-emerald-400 hover:bg-emerald-500/10 rounded-lg transition-colors text-sm font-medium'
            },
            item.label + ' ↓'
        );
    }

    return React.createElement(
        'a',
        { href: item.href, onClick: handleClick, className: 'text-gray-400 hover:text-emerald-500 transition-colors' },
        item.label
    );
}

function MobileMenu({ isOpen, onClose, items, currentPage, setCurrentPage }) {
    if (!isOpen) return null;
    const handleItemClick = (item) => {
        if (item.action) {
            setCurrentPage(item.action);
            window.scrollTo(0, 0);
        } else if (item.external) {
            window.open(item.href, '_blank');
        } else if (item.href && item.href.startsWith('#')) {
            if (currentPage !== 'home') {
                setCurrentPage('home');
                setTimeout(() => {
                    const element = document.querySelector(item.href);
                    if (element) element.scrollIntoView({ behavior: 'smooth', block: 'start' });
                }, 100);
            } else {
                const element = document.querySelector(item.href);
                if (element) element.scrollIntoView({ behavior: 'smooth', block: 'start' });
            }
        }
        onClose();
    };
    return React.createElement(
        'div',
        { className: 'md:hidden fixed top-[73px] left-0 right-0 bg-zinc-950/95 backdrop-blur border-b border-emerald-500/20 shadow-xl z-40' },
        React.createElement(
            'ul',
            { className: 'flex flex-col py-4' },
            items.map((item, idx) =>
                React.createElement(
                    'li',
                    { key: idx },
                    React.createElement(
                        'button',
                        {
                            onClick: () => handleItemClick(item),
                            className: 'block w-full text-left px-6 py-3 text-gray-400 hover:text-emerald-500 hover:bg-zinc-900/50 transition-colors'
                        },
                        item.label + (item.external ? ' ↓' : '')
                    )
                )
            )
        )
    );
}

function Header({ currentPage, setCurrentPage }) {
    const [scrolled, setScrolled] = React.useState(false);
    const [mobileMenuOpen, setMobileMenuOpen] = React.useState(false);

    React.useEffect(() => {
        const handleScroll = () => setScrolled(window.scrollY > 20);
        window.addEventListener('scroll', handleScroll);
        return () => window.removeEventListener('scroll', handleScroll);
    }, []);

    const headerClasses = `fixed top-0 w-full z-50 transition-all duration-300 ${
        scrolled ? 'bg-zinc-950/95 backdrop-blur-md border-b border-emerald-500/20' : 'bg-transparent'
    }`;

    const handleBack = () => {
        if (currentPage === 'project-detail') {
            setCurrentPage('projects');
        } else {
            setCurrentPage('home');
        }
        window.scrollTo(0, 0);
    };

    return React.createElement(
        'header',
        { className: headerClasses },
        React.createElement(
            'nav',
            { className: 'max-w-7xl mx-auto px-6 py-4 flex justify-between items-center' },
            React.createElement(
                'button',
                {
                    onClick: () => { setCurrentPage('home'); window.scrollTo(0, 0); },
                    className: 'text-xl font-bold bg-gradient-to-r from-emerald-500 to-teal-500 bg-clip-text text-transparent hover:from-emerald-400 hover:to-teal-400 transition-all'
                },
                'Isabella Ramos'
            ),
            currentPage === 'home'
                ? React.createElement(
                    React.Fragment,
                    null,
                    React.createElement(
                        'ul',
                        { className: 'hidden md:flex gap-8 items-center' },
                        NAVIGATION_ITEMS.map((item, idx) =>
                            React.createElement(
                                'li',
                                { key: idx },
                                React.createElement(NavigationLink, {
                                    item: item,
                                    currentPage: currentPage,
                                    setCurrentPage: setCurrentPage
                                })
                            )
                        )
                    ),
                    React.createElement(
                        'button',
                        {
                            onClick: () => setMobileMenuOpen(!mobileMenuOpen),
                            className: 'md:hidden text-gray-400 hover:text-emerald-500 transition-colors p-2',
                            'aria-label': 'Menu'
                        },
                        React.createElement(
                            'svg',
                            { className: 'w-6 h-6', fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24' },
                            mobileMenuOpen
                                ? React.createElement('path', {
                                    strokeLinecap: 'round',
                                    strokeLinejoin: 'round',
                                    strokeWidth: 2,
                                    d: 'M6 18L18 6M6 6l12 12'
                                })
                                : React.createElement('path', {
                                    strokeLinecap: 'round',
                                    strokeLinejoin: 'round',
                                    strokeWidth: 2,
                                    d: 'M4 6h16M4 12h16M4 18h16'
                                })
                        )
                    ),
                    React.createElement(MobileMenu, {
                        isOpen: mobileMenuOpen,
                        onClose: () => setMobileMenuOpen(false),
                        items: NAVIGATION_ITEMS,
                        currentPage: currentPage,
                        setCurrentPage: setCurrentPage
                    })
                )
                : React.createElement(
                    'button',
                    {
                        onClick: handleBack,
                        className: 'text-gray-400 hover:text-emerald-500 transition-colors flex items-center gap-2'
                    },
                    '← Voltar'
                )
        )
    );
}
